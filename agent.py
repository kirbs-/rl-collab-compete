...
class Agent():
    ...
    def __init__(self, params):
        ...
        self.learn_cnt = 0
        self.step_cnt = 0
        self.positive_reward_cnt = 0
        self.negative_reward_cnt = 0

    def step(self, state, action, reward, next_state, done):
        # add experience into replay buffer
        # if reward > 0.0 or self.negative_reward_cnt <= self.positive_reward_cnt or self.positive_reward_cnt >= 500:
        self.buffer.append(Experience(state, action, reward, next_state, done))
        
            # if reward > 0.0:
            #     self.positive_reward_cnt += 1
            # else:
            #     self.negative_reward_cnt += 1
        
        # update step cnt
        self.step_cnt += 1

        # update target network after target_update_step_size and if replay buffer has enough data
        if self.step_cnt % self.params.target_update_step == 0 and len(self.buffer) > self.params.batch_size: # and self.positive_reward_cnt >= 500:
            self.learn()
